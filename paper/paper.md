---
title: 'TorchBuq: A Flexible Python Package for Bayesian Neural Networks in PyTorch'
tags:
    - Python
...
`TorchBuq` addresses this by providing an interface that is almost identical to
...
`TorchBuq`. The highlight colors relate user-facing components to their position
in \autoref{design_graph}. \label{code}](code_example.png)

![Design graph of `TorchBuq` colored highlights correspond to their practical
applications in the code example (\autoref{code}). \label{design_graph}](design_graph.png)

`TorchBuq` sacrifices this extreme flexibility to allow nearly fully automating
...
`TorchBuq` is designed around two core aims:
...
application example and internal interactions of `TorchBuq` with the colors
...
overcome in `TorchBuq` by having the layer provide the names and shapes of the
...
![Overview of the major components of `TorchBuq` and corresponding non-Bayesian
components of PyTorch. \label{overview}](content_overview.png "Content overview for
`TorchBuq` and comparison with the interface of `torch.nn`")
...
sampled weights under these two distributions. Therefore, `TorchBuq` provides the
...
primary target of `TorchBuq`, it is kept in mind wherever possible. A core feature
...
`TorchBuq` adds an additional wrapper around the forward pass, which catches the
